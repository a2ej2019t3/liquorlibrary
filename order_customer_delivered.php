...
<section>
    <br><br><br>
<div>
    <p class="contactms">Delivered Items</p>
  ...
  if(isset($_SESSION['user'])){
     ...
     $orders_list =  "SELECT * from orders o where o.buyerID = '".$user['userID']."' AND status = '1'";
                  ...
                  echo '                
                      ...
                      <div class="row">
												<div class="box box-solid">
													<div class="box-header with-border">
														<h4 class="box-title"><i class="fa fa-calendar"></i> <b>Order Summary</b></h4>
													</div>
													<div class="box-body">
														<table class="table table-striped table-hover">
															<thead>
							...
							for ($i = 0; $i < count($orderRow); $i++) {
								$DBsql = new sql;
								$row = $DBsql->getCartItemsInfo($orderRow[$i]["orderID"], $user['userID']);
								// var_dump($row);
								for ($j = 0; $j < count($row); $j++) {
									echo '
																	<tr>
																		...
																		<td>' . $row[$j]["itemID"] . '</td>
																		<td>' . $row[$j]["orderID"] . '</td>
																		<td>' . $row[$j]["quantity"] . '</td> 
																		<td>$' . $row[$j]["totalprice"] . '</td>
																		</tr>';
